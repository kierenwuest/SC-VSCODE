...
exports.checkAndUpdateSfCli = exports.activate = void 0;
const vscode = __importStar(__webpack_require__(1));
const child_process_1 = __webpack_require__(2);
const authenticate_1 = __webpack_require__(3);
const queryOrg_1 = __webpack_require__(8);
const outputs_1 = __webpack_require__(4);
const fileWatcher_1 = __webpack_require__(9);
const newRecord_1 = __webpack_require__(50);
...
const manageSettingsJson_1 = __webpack_require__(51);
const execAsync = (0, util_1.promisify)(child_process_1.exec);
async function activate(context) {
    // Show the output channel on activation
    outputs_1.outputChannel.show(true);
    // Register commands
    const authenticateCommand = vscode.commands.registerCommand('sc-vsc-webdev.authenticate', () => (0, authenticate_1.authenticate)());
    const queryOrgCommand = vscode.commands.registerCommand('sc-vsc-webdev.queryOrg', () => (0, queryOrg_1.queryOrg)());
    const newRecordCommand = vscode.commands.registerCommand('sc-vsc-webdev.newRecord', (uri) => (0, newRecord_1.newRecord)(uri));
    ...
    if (workspaceFolders && workspaceFolders.length > 0) {
        ...
        if (!fs.existsSync(settingsPath) || fs.readFileSync(settingsPath, 'utf8').trim() === '') {
            ...
            settings = (0, types_1.baseSettings)(orgAlias, workspaceFolder, orgStatus);
            fs.writeFileSync(settingsPath, JSON.stringify(settings, null, 2));
            (0, outputs_1.showInfo)('Initialized settings.json with base structure.');
        }
        else {
        ...
        if (!fs.existsSync(launchPath)) {
            ...
            (0, outputs_1.showInfo)('launch.json created with default configuration.');
        }
        ...
        checkAndUpdateSfCli()
            .then(() => {
            const orgAlias = vscode.workspace.getConfiguration('SCWDSettings').get('orgAlias');
            if (!orgAlias) {
            ...
            .then(() => {
            (0, outputs_1.showInfo)("Org status checked.");
            (0, outputs_1.showInfo)("SCWD Ready");
        })
            .catch((error) => {
            (0, outputs_1.showError)(`Error during activation: ${error instanceof Error ? error.message : 'An unexpected error occurred'}`);
        });
    }
    else {
...
async function setupWatchers(context) {
    const fileMappings = await (0, manageSettingsJson_1.readSettingsJson)();
    const orgAlias = vscode.workspace.getConfiguration('SCWDSettings').get('orgAlias');
    (0, outputs_1.showInfo)(`Setting up new watchers for orgAlias: ${orgAlias}`);
    //showInfo(`New mappings: ${JSON.stringify(fileMappings, null, 2)}`);
    if (orgAlias && fileMappings.length > 0) {
        fileMappings.forEach(mapping => {
            (0, outputs_1.showInfo)(`Attaching watcher for path: ${mapping.localPath}`);
            fileWatcher_1.WatcherManager.attach({
        ...
        (0, outputs_1.showError)('No file mappings found to attach watchers.');
    }
}
async function checkCliOrgStatus(orgAlias) {
    const workspaceFolders = vscode.workspace.workspaceFolders;
    if (!workspaceFolders || workspaceFolders.length === 0) {
        (0, outputs_1.showError)("No workspace folder is open. Please open a project folder and try again.");
        return 'Unknown';
    ...
    try {
        ...
        if (stderr) {
            (0, outputs_1.showError)(`Error checking Salesforce CLI status: ${stderr}`);
            return 'Unknown';
        }
        ...
        (0, outputs_1.showError)(`Failed to read settings.json: ${error instanceof Error ? error.message : 'An unexpected error occurred'}`);
        return 'Unknown';
    }
}
async function checkAndUpdateSfCli() {
    try {
        ...
        if (stderr) {
            (0, outputs_1.showError)(`Salesforce CLI update: ${stderr}`);
            return;
        }
        if (stdout.includes('already on version')) {
            (0, outputs_1.showInfo)('Salesforce CLI is already on the latest version.');
        }
        else if (stdout.includes('done')) {
            (0, outputs_1.showInfo)('Salesforce CLI has been updated to the latest version.');
        }
        else {
            (0, outputs_1.showInfo)('Salesforce CLI update status is unknown.');
        }
    }
    catch (error) {
        (0, outputs_1.showError)(`Error checking or updating Salesforce CLI: ${error instanceof Error ? error.message : 'An unexpected error occurred'}`);
    }
}
exports.checkAndUpdateSfCli = checkAndUpdateSfCli;
...
async function authenticate() {
    ...
    const orgAlias = vscode.workspace.getConfiguration('SCWDSettings').get('orgAlias') || alias;
    ...
    (0, child_process_1.exec)(command, (error, stdout, stderr) => {
        ...
        if (stdout.includes("Successfully authorized")) {
            (0, outputs_1.showInfo)(`Authenticated successfully. Org alias set to "${orgAlias}".`);
            ...
            const settings = (0, types_1.baseSettings)(orgAlias, workspaceFolders[0].uri.fsPath, "Connected");
            fs.writeFileSync(settingsPath, JSON.stringify(settings, null, 2));
            (0, outputs_1.showInfo)(`Settings initialized for org alias: ${orgAlias}`);
        }
        else {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.outputChannel = exports.showSuccess = exports.showInfo = exports.showError = exports.clearOutput = exports.showOutput = exports.log = void 0;
const vscode = __importStar(__webpack_require__(1));
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.baseSettings = exports.fileDetailsMapping = void 0;
...
const baseSettings = (orgAlias, workspacePath, orgStatus) => ({
...
async function handleRecords(records, detail, orgAlias) {
    for (const record of records) {
        ...
        try {
            ...
            await (0, manageSettingsJson_1.updateSettingsJson)(fullPath, detail, subDirectory, record.Id);
            fileWatcher_1.WatcherManager.initializeIgnore(fullPath);
            fileWatcher_1.WatcherManager.attach({ localPath: fullPath, salesforceObject: detail.objectType, salesforceField: detail.field, salesforceRecordId: record.Id }, orgAlias);
        }
        catch (err) {
...
const vscode = __importStar(__webpack_require__(1));
const outputs_1 = __webpack_require__(4);
const salesforceAPI_1 = __webpack_require__(10);
const newRecord_1 = __webpack_require__(50); // Ensure correct import
    ...
    static attach(mapping, orgAlias) {
        ...
        const handleFileChange = (uri) => {
            ...
            if (uri.fsPath === mapping.localPath) {
                (0, outputs_1.showInfo)(`File Changed: ${uri.fsPath}`);
                (0, salesforceAPI_1.updateSalesforceRecord)(mapping, uri, orgAlias);
            }
            else {
                (0, outputs_1.showInfo)(`Ignored File Change: ${uri.fsPath}`);
            }
        };
        watcher.onDidChange(handleFileChange);
        watcher.onDidCreate((uri) => this.handleFileCreate(uri, orgAlias));
        (0, outputs_1.showInfo)(`Watcher attached to: ${mapping.localPath}`);
        this.activeWatchers.set(mapping.localPath, watcher);
        this.ignoreInitialChange.add(mapping.localPath);
    }
    static async handleFileCreate(uri, orgAlias) {
        (0, outputs_1.showInfo)(`handleFileCreate triggered for: ${uri.fsPath}`);
        if (this.ignoreInitialChange.has(uri.fsPath)) {
            this.ignoreInitialChange.delete(uri.fsPath);
            (0, outputs_1.showInfo)(`Ignoring initial change for: ${uri.fsPath}`);
            return;
        }
        try {
            // Call newRecord with the URI of the new file
            await (0, newRecord_1.newRecord)(uri);
            (0, outputs_1.showInfo)(`New record creation initiated for: ${uri.fsPath}`);
        }
        catch (err) {
            (0, outputs_1.showError)(`Failed to create new record for ${uri.fsPath}: ${err instanceof Error ? err.message : String(err)}`);
        }
    }
    static initializeIgnore(path) {
        this.ignoreInitialChange.add(path);
    }
    static disposeWatcher(path) {
...
var populate = __webpack_require__(33);
...
module.exports = function () {
  if (!debug) {
    try {
      ...
      debug = __webpack_require__(38)("follow-redirects");
    }
    catch (error) { /* */ }
...
/* harmony export */ });
/* harmony import */ var node_process__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(45);
/* harmony import */ var node_os__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(46);
/* harmony import */ var node_tty__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(47);
...
async function newRecord(uri) {
    ...
    try {
        ...
        const recordId = await (0, salesforceAPI_1.createSalesforceRecord)(fileDetails.objectType, record, orgAlias);
        ...
        fileWatcher_1.WatcherManager.attach(mapping, orgAlias);
        // Update settings.json with the new record details
        await (0, manageSettingsJson_1.updateSettingsJson)(filePath, fileDetails, path.basename(fileDirectory), recordId);
    }
    catch (error) {
...
async function getThemeId(themeName, orgAlias) {
...
function createRecordPayload(details, fileName, fileContent, fileDir, parentId, themeDir, filePath) {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.readSettingsJson = exports.updateSettingsJson = void 0;
const vscode = __importStar(__webpack_require__(1));
const fs = __importStar(__webpack_require__(5));
const path = __importStar(__webpack_require__(6));
const outputs_1 = __webpack_require__(4);
const p_queue_1 = __importDefault(__webpack_require__(52));
const updateQueue = new p_queue_1.default({ concurrency: 1 });
async function updateSettingsJson(localPath, detail, subDirectory, recordId) {
    await updateQueue.add(async () => {
        ...
        try {
            ...
            (0, outputs_1.log)(`Updated settings.json with new file: ${localPath}`);
        }
        catch (error) {
            (0, outputs_1.showError)(`Failed to update settings.json: ${error instanceof Error ? error.message : 'An unknown error occurred'}`);
        }
    });
}
exports.updateSettingsJson = updateSettingsJson;
async function readSettingsJson() {
    ...
    try {
        ...
        const collectFileMappings = (obj, parentKeys = []) => {
            let fileMappings = [];
        ...
        (0, outputs_1.showInfo)(`Successfully read settings.json with file mappings.`);
        return fileMappings;
    }
    catch (error) {
        (0, outputs_1.showError)(`Failed to read settings.json: ${error instanceof Error ? error.message : 'An unknown error occurred'}`);
        return [];
    }
}
exports.readSettingsJson = readSettingsJson;
...
/* harmony export */ });
/* harmony import */ var eventemitter3__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(53);
/* harmony import */ var p_timeout__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(55);
/* harmony import */ var _priority_queue_js__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(56);
...
/* harmony export */ });
/* harmony import */ var _lower_bound_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(57);
